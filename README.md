...
Para probar la aplicación correctamente, es necesario levantar el backend (emuladores) y la aplicación.

### 1. Iniciar Backend (Firebase Emulators)

> ⚠️ **Requisito**: Tener Java instalado.

1.  Ve a `firebase-backend/`:
    En el README de este explica los pasos para iniciarlo.
...
Estructura de directorios más relevantes en `src/`:

```
src/
├── api/                  # Repositorios y servicios externos (Backend)
├── app/                  # Pantallas y rutas (Expo Router)
├── assets/               # Imágenes, fuentes y datos estáticos
├── components/           # UI Kit y componentes compartidos
│   ├── commons/          # Componentes de negocio complejos
│   └── ui/               # Componentes atómicos (Botones, Inputs)
├── context/              # Estado global (Auth, Theme, etc.)
├── entities/             # Definiciones de modelos y tipos de dominio
├── features/             # Lógica de negocio modularizada por funcionalidad
├── hooks/                # Hooks personalizados globales
├── lib/                  # Configuración de terceros (Firebase, React Query)
