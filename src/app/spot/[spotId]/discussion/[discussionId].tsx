      ...
      <VStack className="gap-3">
        ...
        <HStack className="justify-between items-center pt-2 border-t border-gray-100">
          ...
          <Button
            ...
            disabled={
              isSubmitting || (!newComment.trim() && selectedMedia.length === 0)
            }
            className="px-4"
          >
            {isSubmitting ? (
...
const DiscussionInfoHeader = React.memo(function DiscussionInfoHeader({
  ...
  return (
    <View style={{ padding: 12, backgroundColor: "#f9fafb" }}>
      ...
      {isOwner && (
        <Button variant="outline" onPress={onEdit} className="mt-3">
...
export default function DiscussionPage() {
  ...
  const [selectedCommentForReply, setSelectedCommentForReply] =
    React.useState<CommentWithUser | null>(null);

  // Obtener spotId de la discussion cargada
  const spotId = discussion?.details?.spotId;

  const { voteState, handleLike, handleDislike } = useDiscussionVote(
    spotId,
    discussionId,
  ...
  const {
    ...
    contextId: discussion?.details?.spotId || '',
  ...
  const handleLoadReplies = React.useCallback(
    async (commentId: string) => {
      try {
  ...
  const renderItem = React.useCallback(
    ({ item, index }: { item: (typeof comments)[0]; index: number }) => {
      try {
        return (
          <CommentCard
            comment={item}
  ...
  const keyExtractor = React.useCallback(
    (item: (typeof comments)[0]) => item.id,
    []
  );
