...
export default function ChatInfo() {
  ...
  useEffect(() => {
    const loadParticipants = async () => {
      ...
      try {
        const fetched = await Promise.all(chat.memberIds.map(id => userRepository.getUserById(id)));
  ...
  const handleAddMember = async (userIdToAdd: string) => {
    ...
    try {
      ...
      setParticipants(prev => (prev.find(u => u.id === newUser.id) ? prev : [...prev, newUser]));
    } catch {
  ...
  if (isLoading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-white">
        <Text className="text-slate-500">Cargando información...</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView className="flex-1 bg-white">
      <View className="px-4 py-3 border-b border-slate-200 flex-row items-center justify-between">
        <Pressable onPress={() => router.back()} className="px-2 py-1">
          <Text className="text-cyan-600 font-semibold">Atrás</Text>
        </Pressable>
        <Text className="text-lg font-semibold text-slate-900">Información del grupo</Text>
        <View style={{ width: 50 }} />
      </View>

      <View className="px-4 py-4 border-b border-slate-200 flex-row items-center">
        <Avatar size="lg" className={`${chat.photoURL ? 'bg-transparent' : 'bg-cyan-600'}`}>
          {chat.photoURL ? (
        ...
        <View className="flex-1 pl-3">
          <Text className="text-xl font-semibold text-slate-900">{chat.name}</Text>
          {chat.description ? <Text className="text-sm text-slate-600" numberOfLines={2}>{chat.description}</Text> : null}
          <Text className="text-xs text-slate-500 pt-1">{chat.memberIds.length} miembros</Text>
        </View>
      </View>

      <View className="px-4 py-3 border-b border-slate-100">
        <View className="flex-row items-center justify-between pb-2">
          <Text className="text-sm font-semibold text-slate-900">Participantes</Text>
          {isAdmin ? (
            <Button action="primary" size="sm" onPress={() => setIsAddModalOpen(true)}>
        ...
        <View className="pt-2">
          <Button action="negative" variant="outline" size="md" onPress={handleLeaveGroup}>
            <ButtonText>Salir del grupo</ButtonText>
          </Button>
        </View>
      </View>
