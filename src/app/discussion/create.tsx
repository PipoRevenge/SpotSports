...
export default function DiscussionCreatePage() {
  ...
  const handleSubmit = async (payload: { title: string; description?: string; tags?: string[]; media?: any[] }) => {
    ...
    try {
      const newDiscussion = await createDiscussion(user.id, { ...payload, spotId });
      if (!newDiscussion) {
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
        <DiscussionForm onSubmit={handleSubmit} isSubmitting={isCreating} spotSports={sportRatings?.map(sr => ({ id: sr.sportId, name: sr.sportName })) ?? []} />
    </SafeAreaView>
  );
}
