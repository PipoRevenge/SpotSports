...
import { useFollowing } from '@/src/features/relationships/hooks/use-following';
import { router, useLocalSearchParams } from 'expo-router';
import React, { useEffect, useState } from 'react';

export default function FollowingPage() {
  const { userId } = useLocalSearchParams<{ userId: string }>();
  const { users, isLoading, isLoadingMore, error, refetch, loadMore } = useFollowing(userId);
  ...
  return (
    <View className="flex-1 p-4">
      <Text className="text-lg font-bold pb-2">Siguiendo</Text>
