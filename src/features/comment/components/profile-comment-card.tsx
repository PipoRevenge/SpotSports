...
export const ProfileCommentCard: React.FC<ProfileCommentCardProps> = ({
  ...
  const [authorUser, setAuthorUser] = React.useState<{
    name?: string;
    photo?: string;
  } | null>(null);
  ...
  const sourceTitle = (() => {
    ...
    if (comment.sourceType === "discussion" && discussion) {
      return discussion.details.title || "Discussion";
    }
    return comment.sourceType === "review" ? "Review" : "Discussion";
  })();

  const SourceIcon = comment.sourceType === "review" ? FileText : MessageCircle;

  const replyTargetLabel = React.useMemo(() => {
    if (comment.parentId && parentComment) return "comment";
    if (comment.sourceType === "review") return "review";
    if (comment.sourceType === "discussion") return "discussion";
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setParentUser({
          name: u?.userDetails?.userName || u?.userDetails?.fullName,
          photo: u?.userDetails?.photoURL,
        });
      } catch {
        if (mounted) setParentUser(null);
      }
  ...
  return (
    <Pressable onPress={handlePress}>
      <Card className="bg-white px-4 py-3 shadow-sm mb-3 rounded-xl">
        {/* Reply target label */}
        <Text className="text-xs text-gray-500 font-medium mb-2">{`Replying to: ${replyTargetLabel}`}</Text>
            ...
            <HStack className="items-start gap-3 mb-1">
              <Avatar size="xs" className="bg-gray-100 border border-gray-200">
                {parentUser?.photo ? (
                  ...
                  <AvatarFallbackText className="text-gray-700 font-semibold">
                    {getInitials(parentUser?.name || parentComment.userId)}
                  </AvatarFallbackText>
                )}
              </Avatar>
              <VStack className="flex-1">
                <HStack className="items-center justify-between">
                  ...
                  <Text className="text-xs text-gray-400">
                    {formatDateTime(parentComment.createdAt)}
                  </Text>
                </HStack>
                <Text
                  className="text-sm text-gray-700 italic mt-1"
                  numberOfLines={3}
                >
        ...
        <HStack className="items-center justify-between mb-2">
            ...
            <Avatar size="sm" className="bg-blue-100 border border-blue-200">
              {authorUser?.photo ? (
                ...
                <AvatarFallbackText className="text-blue-700 font-semibold">
                  {getInitials(authorUser?.name || comment.userId)}
                </AvatarFallbackText>
              )}
            </Avatar>
            <VStack>
              <Text className="text-sm font-semibold">
                {authorUser?.name || "User"}
              </Text>
          ...
          <VStack className="items-end">
            ...
            <HStack className="items-center gap-1">
              ...
              <Text className="text-xs text-gray-500">in: {sourceTitle}</Text>
            </HStack>
          </VStack>
        </HStack>
