    ...
    <Pressable onPress={handlePress}>
      <Card className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm mb-3">
        <HStack className="justify-between items-start mb-2">
          ...
          {review.details.rating != null && (
            <VStack className="items-end">
              <RatingStars rating={review.details.rating} maxStars={5} size="sm" />
              <Text className="text-sm font-semibold text-gray-800">{review.details.rating.toFixed(1)} ⭐</Text>
            </VStack>
          )}
        </HStack>
        ...
        <VStack className="gap-1 mb-3">
          {
            (() => {
              ...
              const headlineRaw = lines[0] || 'Reseña';
              const headline = headlineRaw.length > 70 ? `${headlineRaw.slice(0, 67)}...` : headlineRaw;
              const remainder = lines.slice(1).join('\n');
              return (
                <>
                  <Text className="text-sm text-gray-900 font-semibold" numberOfLines={2}>{headline}</Text>
                  {remainder ? (
                    <Text className="text-sm text-gray-700" numberOfLines={3}>{remainder}</Text>
                  ) : null}
                </>
              );
            })()
            ...
            <View className="border border-gray-200 rounded-lg overflow-hidden w-full">
              <SportsRatingTable
                sports={review.details.reviewSports.map(sport => ({
                  ...
                  sportName: (getSportName ? getSportName(sport.sportId) : globalGetSportName(sport.sportId)) ?? sport.sportId,
        ...
        <HStack className="mt-3 items-center justify-between">
          ...
          <Text className="text-xs text-blue-600">Ver publicación</Text>
        </HStack>
      </Card>
    </Pressable>
  );
