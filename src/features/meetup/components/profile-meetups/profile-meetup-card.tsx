...
export const ProfileMeetupCard: React.FC<ProfileMeetupCardProps> = ({
  meetup,
  isOwn = false,
  saved = false,
  onOpen,
  ...
  const isParticipant =
    Array.isArray(meetup?.participants) && currentUser?.id
      ? meetup.participants.includes(currentUser.id)
      : false;
  ...
  const participantsCount =
    typeof meetup?.participantsCount === "number"
  ...
  const sportName = sportId
    ? getSportName(sportId) ?? sportNames?.[sportId] ?? sportId
    : undefined;
  const firstMedia =
    spot?.details?.media && spot.details.media.length > 0
      ? spot.details.media[0]
      : meetup?.media && meetup.media.length > 0
      ? meetup.media[0]
      : undefined;
  ...
  const handleLeave = async () => {
    ...
    if (isOrganizer) {
      const confirmed = await showConfirm(
        "Leave meetup",
        "⚠️ You are the organizer of this meetup. If you leave, the meetup will be permanently deleted and all participants will be disconnected. Are you sure?",
        "Delete meetup",
        "Cancel"
      );
      if (confirmed) {
          ...
          {isOwn && isParticipant && (
            ...
            >
              <HStack className="gap-2 items-center">
                ...
                <Text className="text-xs font-medium text-red-600">Leave</Text>
              </HStack>
            </Button>
          )}
