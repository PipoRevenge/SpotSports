...
export const ProfileMeetupsList: React.FC<{ userId?: string }> = ({
  userId,
}) => {
  ...
  const serverItems: any[] = Array.isArray(server.data)
    ? server.data
    : server.data
    ? Object.values(server.data)
    : [];
  const localItems: any[] = Array.isArray(local.items)
    ? local.items
    : local.items
    ? Object.values(local.items)
    : [];
  ...
  for (const s of serverItems) {
    combinedMap.set(s.id, {
      ...s,
      _saved: localItems.some((l) => l.id === s.id),
    });
  }
  ...
  const [sortOrder, setSortOrder] = useState<"date-asc" | "date-desc">(
    "date-asc"
  );
  const sortOptions = [
    { key: "date-asc", label: "Date — Soonest first" },
    { key: "date-desc", label: "Date — Furthest first" },
  ];
  ...
  if (isOwn && currentUser?.id) {
    ...
    const joined = items
      .filter(
        (m) => Array.isArray(m?.participants) && m.participants.includes(userId)
      )
      .sort((a, b) => {
    ...
    const others = items
      .filter(
        (m) =>
          !Array.isArray(m?.participants) || !m.participants.includes(userId)
      )
      .sort((a, b) => {
        const da = toDate(a?.date ?? a?.nextDate)?.getTime() ?? 0;
        const db = toDate(b?.date ?? b?.nextDate)?.getTime() ?? 0;
  ...
  console.debug(
    "[ProfileMeetupsList] combinedCount=",
    combined.length,
    "serverCount=",
    serverItems.length,
    "localCount=",
    ...
    return (
      <VStack className="p-4 items-center">
        <Text className="text-gray-600">
          {isOwn
            ? "You have no saved meetups yet."
            : "No meetups found for this user."}
        </Text>
      </VStack>
    );
    ...
    <VStack className="p-2 gap-2">
      <HStack className="justify-between items-center px-2">
        <Text className="text-sm font-semibold">
          {isOwn ? "Saved meetups" : "Participating in"}
        </Text>

        <Select
          selectedValue={sortOrder}
          onValueChange={(v) => setSortOrder(v as "date-asc" | "date-desc")}
        >
          <SelectTrigger variant="outline" size="sm" className="min-w-[180px]">
            <SelectInput
              placeholder="Sort"
              value={sortOptions.find((o) => o.key === sortOrder)?.label}
            />
        ...
        <ProfileMeetupCard
          ...
          onLeave={async () => {
            try {
              ...
              console.debug(
                "[ProfileMeetupsList] leaving meetup",
                m.id,
                "user=",
                currentUser.id
              );
