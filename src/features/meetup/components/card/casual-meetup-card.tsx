...
import { CasualMeetup, DEFAULT_MEETUP_PARTICIPANT_LIMIT } from '@/src/entities/meetup';
...
export const CasualMeetupCard: React.FC<CasualMeetupCardProps> = ({ meetup, onPress, onJoin, onLeave, isJoining, isLeaving, isParticipant, isRequested }) => {
  ...
  const formatDateTime = (v: any) => {
    ...
    if (v && typeof v.seconds === 'number') return new Date(v.seconds * 1000).toLocaleString();
    return '';
  };
    ...
    <Pressable onPress={() => onPress(meetup.id)}>
      <Card className="p-4 mb-4 bg-white rounded-lg shadow-sm">
        ...
        <View className="flex-row justify-between items-center">
          <Text className="text-xs font-bold text-blue-600 uppercase tracking-wider">{(meetup as any).type === 'ROUTINE' ? 'Routine' : 'Casual Play'}</Text>
          <Text className="text-xs text-gray-500">{formatDateTime(((meetup as any).type === 'ROUTINE' && (meetup as any).nextDate) ? (meetup as any).nextDate : meetup.date)}</Text>
        </View>
        ...
        <View className="flex-row gap-3 mt-4">
          ...
          {isParticipant ? (
            onLeave ? (
              ...
              >
                <ButtonText className="text-red-500">{isLeaving ? 'Leaving...' : 'Leave'}</ButtonText>
              </Button>
            ) : null
          ) : isRequested ? (
            <Button variant="outline" className="flex-1 border-slate-300" isDisabled>
            ...
            onJoin ? (
              ...
              >
                <ButtonText className="text-primary-500">{isJoining ? 'Joining...' : 'Join'}</ButtonText>
              </Button>
            ) : null
          )}
        </View>
