...
import { DEFAULT_MEETUP_SORT, MEETUP_SORT_OPTIONS } from '../../constants/sort-options';
...
interface MeetupListProps {
  ...
  onRegisterFiltersControls?: (controls: { open: () => void; getActiveFilters: () => number; setSort?: (s: MeetupSortOptions) => void }) => void;
  /** Optional header slot to allow parent to render consistent controls */
  headerSlot?: React.ReactNode;
}

export const MeetupList: React.FC<MeetupListProps> = ({ spotId, onRegisterFiltersControls, headerSlot }) => {
  ...
  const [sort, setSort] = React.useState<MeetupSortOptions>({ field: DEFAULT_MEETUP_SORT });
  ...
  return (
    <View className="flex-col gap-4">
      ...
      {headerSlot === null ? null : (headerSlot ?? (
      <View className="gap-2 pb-2">
        <HStack className="justify-between items-center">
          <Text className="text-lg font-semibold">
            {meetups?.length || 0} {meetups?.length === 1 ? 'Meetup' : 'Meetups'}
          </Text>
          <View className="relative">
            ...
            {activeFilters > 0 ? (
              <View className="absolute -top-1 -right-1 bg-red-500 rounded-full h-4 w-4 items-center justify-center">
                <Text className="text-white text-[10px] font-bold">{activeFilters}</Text>
              </View>
            ) : null}
          </View>
        </HStack>
