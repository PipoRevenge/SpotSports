...
export const RelationshipListItem: React.FC<RelationshipListItemProps> = ({ user, onNavigateToProfile, onFollowChange }) => {
      ...
      <Pressable onPress={handleNavigate} style={{ flex: 1 }}>
        <HStack space="md" className="items-center">
          ...
          {user.userDetails.photoURL ? (
            ...
            <AvatarFallbackText>{user.userDetails.fullName || user.userDetails.userName || 'U'}</AvatarFallbackText>
          )}
          </Avatar>
          <VStack className="flex-1">
          <Text className="font-bold">{user.userDetails.fullName || 'Nombre'}</Text>
